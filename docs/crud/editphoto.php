...
if (isset($_GET['id_foto'])) {
    ...
    // Fetch the photo only if the current session user is the owner
    $stmt = $conn->prepare("SELECT * FROM foto WHERE id_foto = :id_foto AND id_user = :id_user");
    $stmt->execute([':id_foto' => $id_foto, ':id_user' => $id_user]);
    $foto = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$foto) {
        echo "No image found with that ID or you do not have permission to edit it!";
        exit;
    }
} else {
    echo "No ID provided!";
...
if (isset($_POST['daftar']) && $foto) {
    ...
    if (empty($error)) {
        ...
        $sql = 'UPDATE foto SET judul_foto = ?, deskripsi = ?, tgl_unggah = ?, id_album = ?' . ($updateImage ? ', lokasi_file = ?' : '') . ' WHERE id_foto = ? AND id_user = ?';
        $params = [$judul_foto, $deskripsi, $tgl_unggah, $id_album, $id_foto, $id_user];
        if ($updateImage) {
